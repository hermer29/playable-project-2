...
foreach ($mainScript in $mainScripts) {
    ...
    $sceneLaunchPattern = "cc\.director\.once\(cc\.Director\.EVENT_AFTER_SCENE_LAUNCH, function \(\) \{\s*splash\.style\.display = 'none';\s*(?:if \(window\.__applyBlastCanvasSize\) \{[\s\S]*?\}\s*)?\}\);"
    $sceneLaunchReplacement = @"
...
"@
    $patchedScriptContent = [regex]::Replace(
        $patchedScriptContent,
        $sceneLaunchPattern,
        $sceneLaunchReplacement
    )
